...
const EmployeeUploadPhoto = () => {
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            formData.append("file", file); // имя параметра = "file", как в @RequestParam("file")

            const res = await fetch(
                "http://localhost:8080/api/profile/employee/set-photo",
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`, // ВАЖНО: только авторизация, БЕЗ Content-Type
                    },
                    body: formData,
                }
            );
            ...
            if (!res.ok) {
                // попробуем показать то, что реально прислал бэк
                let message = text;
                try {
                ...
                <section className="emp-card">
                    ...
                    <div className="emp-form">
                        <p style={{ color: "#fff", fontSize: 14, marginBottom: 8 }}>
                            Загрузите вашу главную фотографию (портрет).
                        </p>
